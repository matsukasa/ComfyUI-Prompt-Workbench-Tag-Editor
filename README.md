...
「選択した項目」では、タグ編集画面で選択中のタグと、タグセット編集画面でチェックしたタグセットだけを書き出します。
タグカタログのみ書き出す場合は選択中のタグだけ、タグセットのみ書き出す場合はチェック済みタグセットだけ、両方書き出す場合はその両方を使います。
選択した項目がない場合はZIPを書き出せません。
...
Export previewには、書き出し予定のカタログ操作数、タグセット操作数、Export対象件数、削除操作を含めないことを表示します。
